          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-right px-3 py-3 font-medium text-slate-600 text-xs uppercase tracking-wide">Answered</th>
                <th className="text-right px-3 py-3 font-medium text-slate-600 text-xs uppercase tracking-wide">N/A 1x</th>
                <th className="text-right px-3 py-3 font-medium text-slate-600 text-xs uppercase tracking-wide">N/A 2x</th>
              ...
              {sorted.map((r) => (
                <>
                  <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-700">{r.agentName || '—'}</td>
                    <td className="px-3 py-3 text-right text-slate-700">{r.answered}</td>
                    <td className="px-3 py-3 text-right text-slate-500">{r.noAnswer}</td>
                    <td className="px-3 py-3 text-right text-slate-500">{r.noAnswer2x}</td>
                        ...
                        <div className="grid grid-cols-2 gap-6">
                          <div>
                            ...
                            {labels.length === 0 || Object.keys(r.rejectionBreakdown).length === 0 ? (
                              <p className="text-sm text-slate-400">No rejection data</p>
                            ) : (
                          ...
                          {r.remarks && (
                            <div>
                              ...
                              <p className="text-sm text-slate-700 whitespace-pre-wrap">{r.remarks}</p>
                            </div>
                          )}
                        </div>
                      </td>
